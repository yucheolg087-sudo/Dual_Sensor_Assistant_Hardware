...
- 기술: 초음파(상단) + ToF(하단) 센서 융합  감지 알고리즘 적용
- 성과:   U7 메이커스 경진대회 대상
...
본 프로젝트는 이와 같이 보행자가 인지하기 어려운 사각지대의 위험 요소를 실시간으로 파악하고 버저로 즉각적인 알림을 제공하여 보행자의 안전을 지키는 하드웨어(디바이스)을 개발하였습니다.

* 개발 기간: 2025.09 ~ 2026.01
* 팀 구성: 3인 
* 담당 역할: 팀장 
    * 시스템 통합: 센싱 처리 및 전체 회로 설계
    * 알고리즘 구현: 낙상 감지를 위한 미분 로직 및 EMA 필터 구상 및 적용
...
Q3. 지팡이 부착형이 아닌 '웨어러블' 방식을 택한 이유는?

시각장애인은 보행 시 이미 한 손에 흰 지팡이를 사용해야 합니다. 기기를 지팡이에 부착하면 무게 중심이 쏠려 손목 피로도를 유발하고, 별도의 핸드형 기기를 만들면 양손이 모두 구속되는 문제가 생깁니다.
기존 보행 습관을 방해하지 않으면서 추가적인 안전을 확보하기 위해, 두 손이 자유로운(Hands-free) 벨트/스트랩 착용 방식을 최종적으로 결정했습니다.
